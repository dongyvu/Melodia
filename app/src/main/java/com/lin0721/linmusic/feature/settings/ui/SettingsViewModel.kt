...
class SettingsViewModel(
    ...
    val preferAmllLyrics = settingsPreferences.preferAmllLyrics.asState(true)

    val showLyricTranslation = settingsPreferences.showLyricTranslation.asState(true)

    val showLyricRomanization = settingsPreferences.showLyricRomanization.asState(false)

    private val _lyricsCacheSize = MutableStateFlow(lyricsCache.diskSizeBytes())
    val lyricsCacheSize = _lyricsCacheSize.asStateFlow()
    ...
    fun updatePreferAmllLyrics(enabled: Boolean) = launchSave { settingsPreferences.savePreferAmllLyrics(enabled) }

    fun updateShowLyricTranslation(enabled: Boolean) = launchSave { settingsPreferences.saveShowLyricTranslation(enabled) }

    fun updateShowLyricRomanization(enabled: Boolean) = launchSave { settingsPreferences.saveShowLyricRomanization(enabled) }

    fun clearLyricsCache() {
        viewModelScope.launch(Dispatchers.IO) {
            lyricsCache.clear()
            _lyricsCacheSize.value = 0L
            _toastEvent.emit("歌词缓存已清除")
        }
    }

    fun refreshLyricsCacheSize() {
        viewModelScope.launch(Dispatchers.IO) {
            _lyricsCacheSize.value = lyricsCache.diskSizeBytes()
        }
    }

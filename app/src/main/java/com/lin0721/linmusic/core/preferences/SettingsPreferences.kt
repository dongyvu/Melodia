...
class SettingsPreferences(private val context: Context) {
    ...
    companion object {
        ...
        private val KEY_PREFER_AMLL_LYRICS = booleanPreferencesKey("prefer_amll_lyrics")
        private val KEY_SHOW_LYRIC_TRANSLATION = booleanPreferencesKey("show_lyric_translation")
        private val KEY_SHOW_LYRIC_ROMANIZATION = booleanPreferencesKey("show_lyric_romanization")
    ...
    val preferAmllLyrics: Flow<Boolean> = context.settingsDataStore.data.map { prefs ->
        prefs[KEY_PREFER_AMLL_LYRICS] ?: true
    }

    suspend fun savePreferAmllLyrics(enabled: Boolean) {
        context.settingsDataStore.edit { it[KEY_PREFER_AMLL_LYRICS] = enabled }
    }

    val showLyricTranslation: Flow<Boolean> = context.settingsDataStore.data.map { prefs ->
        prefs[KEY_SHOW_LYRIC_TRANSLATION] ?: true
    }

    suspend fun saveShowLyricTranslation(enabled: Boolean) {
        context.settingsDataStore.edit { it[KEY_SHOW_LYRIC_TRANSLATION] = enabled }
    }

    val showLyricRomanization: Flow<Boolean> = context.settingsDataStore.data.map { prefs ->
        prefs[KEY_SHOW_LYRIC_ROMANIZATION] ?: false
    }

    suspend fun saveShowLyricRomanization(enabled: Boolean) {
        context.settingsDataStore.edit { it[KEY_SHOW_LYRIC_ROMANIZATION] = enabled }
    }
